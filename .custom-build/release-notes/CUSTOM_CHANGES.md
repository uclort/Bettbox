...
- 选中请求或连接后展开底部详情，顶部拖拽手柄可调整面板高度。通用页使用占满详情视口的响应式布局，地址和进程信息独占整行，所有长文本自动换行且可框选复制；显式滚动条固定在面板最右侧，并提供复制详情按钮。信息区分客户端地址、目标地址、内核 Fake-IP、实际出站本地地址和直连目标/代理入口地址，GeoIP/ASN 仅在 TrackerInfo 或现有核心 GeoIP 查询返回值时显示。原“计时 & 日志”改为“Mihomo 链路”，链路固定先展示当前请求目标和目标解析方式，本地未解析的代理目标明确标记为“真实解析交由代理远端完成”，不再将节点 DNS 冒充为当前目标解析；目标域名与代理节点服务器的 DNS 事件分别标记为“目标 DNS / 节点 DNS”，策略链按“策略组 → 中间组 → 最终节点”顺序展示；链路中的节点 DNS 成功和缓存命中同步进入 DNS 页“运行缓存 / 出站节点”分类。链路文字支持选择复制，并优先展示 custom-mihomo 随当前连接返回的 DNS 逐服务器尝试/成功/失败、规则匹配、策略链和真实 socket 建立事件，旧内核才退回源端口与目标精确关联日志；Mihomo 不提供的 HTTP 请求/响应报头和正文页签已移除。
