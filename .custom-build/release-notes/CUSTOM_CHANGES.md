...
- Android 在“更多”中以“工具”替换原“面板”入口，并以移动端目录分别进入最近请求、活动连接、DNS、设备、流量统计、日志和 Sub-Store；macOS 独立面板复用同一工具目录和移动端单列页面，不再维护独立桌面表格分支。请求与连接使用纵向卡片惰性列表，点击后原位切换到占满页面的通用信息与 Mihomo 链路详情，详情顶部只显示目标链接，客户端信息保留在通用详情中，右上角三点菜单复用完整的生成规则、添加覆写、补充 Sub-Store 和复制能力；DNS、设备、流量和日志使用移动端单列布局。所有页面继续复用主应用 Provider 或独立面板现有 IPC、Mihomo 核心与刷新状态，不创建第二套数据服务。
