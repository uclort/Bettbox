...
- 托盘菜单与代理面板共享测速状态并持续显示测速进度和结果；每次测速请求均独立执行，不复用其他入口的在途任务或结果。
...
- 所有平台和测速入口均取消节点测速并发上限：Dart 使用 `Future.wait` 立即提交列表中的全部项目，桥接层直接启动独立请求，Mihomo 策略组与 Provider 健康检查为每个项目启动独立 goroutine；不保留全局 16 并发队列或 Provider 的 10 并发限制。配置中旧 `concurrencyLimit` 字段仅为兼容已有本地数据而保留，运行时不再读取。
- Dart、桥接层和 Mihomo 均不再按节点或测试地址去重、缓存、防抖或合并在途请求；列表中的重复节点会分别测速，不同策略组和后台健康检查可以同时提交。桥接旧 `mBatch` 队列及无人读取的结果缓存已删除，仅同一组按钮在自身批次运行期间禁用，避免用户重复点击同一按钮。
- Mihomo 在配置代际切换时显式关闭旧 Proxy Provider，立即取消旧健康检查和拉取任务，避免覆盖安装或首次启动时新旧 Provider 同时测速。
- Mihomo `Proxy.URLTest` 直接执行单次网络测速；每个调用拥有独立 context 与超时，调用方已有更短 deadline 时保持调用方 deadline，否则应用手动或 Provider 传入的测速超时。不存在排队等待时间，也不会因共享任务的失败让多个节点同时秒失败。
- 测速成功或失败均不缓存、不复用且不增加重试；配置重载或 Provider 更新仍会关闭旧 Provider 并阻止旧节点发布过期 hook，新节点立即开始自己的健康检查。
- 回归测试位于 `adapter/urltest_test.go`，覆盖相同节点并发请求互不合并、32 个网络请求同时启动、每请求独立超时、完成和失败后再次真实测速及旧节点拒绝新任务；Dart 回归测试位于 `test/views/proxies/delay_test_coordinator_test.dart`，覆盖重复项目独立提交、多策略组并行和同组重复点击保护。
