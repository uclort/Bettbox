...
class _RuntimeOpaque {
  ...
  int addDartObject(JSRef obj) {
    final id = _nextDartObjectId++;
    _dartObjects[id] = obj;
    return id;
  }

  bool removeDartObject(int id) {
    if (_dartObjects.containsKey(id)) {
      _dartObjects.remove(id);
      return true;
    }
    return false;
  }

  JSRef? getDartObject(int id) {
    return _dartObjects[id];
  }
}
...
Pointer<JSRuntime> jsNewRuntime(
  ...
  if (rt == nullptr) {
    throw Exception('Failed to allocate JS Runtime from C');
  }
...
void jsFreeRuntime(
  ...
  if (opaque != null) {
    while (true) {
      final refId = opaque._dartObjects.keys.firstWhereOrNull((k) => opaque._dartObjects[k] is JSRefLeakable);
      if (refId == null) break;
      final ref = opaque._dartObjects[refId]!;
      ref.destroy();
      opaque._dartObjects.remove(refId);
    }
    while (opaque._dartObjects.isNotEmpty) {
      final refId = opaque._dartObjects.keys.first;
      final ref = opaque._dartObjects[refId]!;
      final objStrs = ref.toString().split('\n');
      final objStr = objStrs.length > 0 ? objStrs[0] + " ..." : objStrs[0];
      referenceleak.add(
          "  $refId\t${ref._refCount + 1}\t${ref.runtimeType.toString()}\t$objStr");
      ref.destroy();
      opaque._dartObjects.remove(refId);
    }
  }
